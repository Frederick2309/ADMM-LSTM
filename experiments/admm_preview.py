...
example_parameter_dictionary: Dict[str, Dict[str, Dict[str, float]]] = {
    'GoogleStock': {
        'rho': {
            'i': 1, 'f': 1, 'g': 1, 'o': 1, 'c': 0.007, 'h': 0.0004, 'y': 0.00004
        },
        'beta': {
            'wi': 7e-4, 'vi': 7e-4, 'wf': 7e-4, 'vf': 7e-4, 'wg': 7e-4, 'vg': 7e-4, 'wo': 7e-4, 'vo': 7e-4, 'wy': 7e-4
        }
    },
    'GEFCOM2012': {
        'rho': {
            'i': 10., 'f': 10., 'g': 10., 'o': 10., 'c': 0.02, 'h': 0.001, 'y': 0.0001
        },
        'beta': {
    ...
    'YahooFinance': {
        'rho': {
            'i': 1., 'f': 1., 'g': 1., 'o': 1., 'c': 0.01, 'h': 0.001, 'y': 0.0001
        },
        'beta': {
            'wi': 8e-7, 'vi': 8e-7, 'wf': 8e-7, 'vf': 8e-7, 'wg': 8e-7, 'vg': 8e-7, 'wo': 8e-7, 'vo': 8e-7, 'wy': 8e-7
        }
...
class ADMMBasedOptimizer(object):
    ...
    def __initialize_normalization_factors(self, param_dict: Dict[str, Dict[str, float]]) -> None:
        ...
        def assertion(key):
            log_assert(isinstance(beta_dict[key], float or int), f'Beta {key} must be a float or integer.')
    ...
    def __update_wy(self) -> None:
        ...
        theta = 0.01
        gradient = rho_y * (h.T @ (h @ wy - a))

        def original_func(beta_t: torch.Tensor) -> torch.Tensor:
            return 0.5 * rho_y * self.__fro_quad(h @ beta_t - a) + 0.5 * self.__get_beta_y() * self.__fro_quad(beta_t)
        ...
        def estimated_func(beta_t: torch.Tensor, theta_t: float) -> torch.Tensor:
            return (original_func(beta_t) + self.__inner_product(gradient, beta_t - wy)
                    + 0.5 * theta_t * self.__fro_quad(beta_t - wy) + 0.5 * self.__get_beta_y() * self.__fro_quad(beta_t))

        def compute_wy(theta_t: float) -> torch.Tensor:
            return (theta_t * wy - gradient) / (theta_t + 2 * self.__get_beta_y())
    ...
    def __update_weights(self, map_from: str, map_to: str) -> None:
        ...
        def original_func(beta_t: torch.Tensor) -> torch.Tensor:
            ...
            return func_value + 0.5 * self.__get_beta(map_from, map_to) * self.__fro_quad(beta_t)

        def estimated_func(beta_t: torch.Tensor, theta_t: float) -> torch.Tensor:
            return (original_func(w) + torch.sum(gradient * (beta_t - w))
                    + self.seq_len * 0.5 * theta_t * self.__fro_quad(beta_t - w)
                    + 0.5 * self.__get_beta(map_from, map_to) * self.__fro_quad(beta_t)
            )

        def compute_beta(theta_t: float) -> torch.Tensor:
            return (
                (0.5 * rho_gate * self.seq_len * theta_t * w - gradient)
                / (self.__get_beta(map_from, map_to) + 0.5 * rho_gate * theta_t * self.seq_len)
            )

        theta = 1
        beta = compute_beta(theta)
        ...
        self.__set_weight(map_from, map_to, compute_beta(theta))
    ...
    def __update_primal_i_f_g_o(self, gate: str, t: int) -> None:
        ...
        self.__set_gate(gate, t, - (
                lam_var - rho_var * act_func(act_value) + rho_var2 * (p2 * p3 - lam_var2 / rho_var2 - var2) * p1
        ) / (rho_var + rho_var2 * p1 * p1))
    ...
    def __update_primal_c(self, t: int) -> None:
        ...
        ) + gradient - 0.5 * rho_h * theta * c) / (rho_c + 0.5 * theta * rho_h))
    ...
    def __update_primal_h(self, t: int) -> None:
        ...
        def original_func(beta_t: torch.Tensor) -> torch.Tensor:
            return 0.5 * rho_h * self.__fro_quad(beta_t @ wy - a) + 0.5 * rho_h * self.__fro_quad(beta_t - o * tanh_c + lam_h / rho_h)

        def estimated_func(beta_t: torch.Tensor, theta_t: float) -> torch.Tensor:
            return (original_func(h) + torch.sum(gradient * (beta_t - h))
                    + 0.5 * theta_t * self.__fro_quad(beta_t - h) + 0.5 * rho_h * self.__fro_quad(beta_t - o * tanh_c + lam_h / rho_h))

        def compute_beta(theta_t: float) -> torch.Tensor:
            return (
                (theta_t * h + rho_h * o * tanh_c
                 - lam_h - gradient) / (theta_t + rho_h)
            )
        ...
        while original_func(beta) > estimated_func(beta, theta):
            ...
            # beta = gradient / theta
        theta /= 2
        self.__set_gate('h', t, compute_beta(theta))

    def __update_primal_a(self) -> None:
        rho_y = self.__get_rho('y')
        self.__set_a(
            (rho_y * (self.__get_gate('h', self.seq_len) @ self.__get_wy()) + 2 * self.train_y)
            / (2 + rho_y)
        )
